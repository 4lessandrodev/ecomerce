...
class ContatoModel {
     ...
     enviarMensagem(mensagem) {
          return new Promise((resolve, reject) => {
               conect.query(
                    `INSERT INTO tb_contatos(nome, email, mensagem, mensagem_lida) VALUES(?,?,?,?)`,
                    [
                         mensagem._nome,
                         mensagem._email,
                         mensagem._mensagem,
                         mensagem._mensagem_lida,
                    ],
                    (err, result) => {
                         if (err) {
     ...
     listarMensagens() {
          return new Promise((resolve, reject) => {
               conect.query(
                    `SELECT *, DATE_FORMAT(data_envio, '%d/%m/%Y %H:%i:%s') AS data_hora FROM tb_contatos`,
                    (err, result) => {
                         if (err) {
                              console.log(err.message);
                              reject(err.message);
     ...
     marcarComoLida(mensagem) {
          return new Promise((resolve, reject) => {
               conect.query(
                    `UPDATE tb_contatos SET mensagem_lida = ?`,
                    [mensagem._mensagem_lida, mensagem._id],
                    (err, result) => {
                         if (err) {
                              console.log(err.message);
                              reject(err.message);
