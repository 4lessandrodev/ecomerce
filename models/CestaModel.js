...
class CestaModel {
     constructor(
          ...
          alteracoes_permitidas,
          imagem = '/uploads/images/no-image.jpeg',
          status = 1,
          cesta_excluida = 0,
      ...
      INSERT INTO tb_cestas(imagem, descricao, id_categoria_cesta, status, preco,
        ...
        VALUES(?,?,?,?,?,?,?,?)`,
                    [
                         cesta._imagem,
                         cesta._descricao,
                         cesta._id_categoria_cesta,
                         cesta._status,
                         cesta._preco,
                         cesta._alteracoes_permitidas,
                         cesta.__informacoes_nutricionais,
                         cesta._cesta_excluida,
                    ],
                    (err, result) => {
                         if (err) {
     ...
     listarDescricaoECodigoCestas() {
          return new Promise((resolve, reject) => {
               conect.query(
                    `SELECT cesta.id, cesta.descricao, categ.descricao AS categoria FROM tb_cestas cesta 
        INNER JOIN tb_categoria_cestas categ ON cesta.id_categoria_cesta = categ.id
        WHERE cesta.cesta_excluida = 0`,
                    (err, result) => {
                         if (err) {
                              console.log(err);
                              reject(err.message);
     ...
     listarTodasCestas(cesta) {
          return new Promise((resolve, reject) => {
               conect.query(
                    `SELECT c.imagem, c.id, c.descricao, cc.descricao AS 'categoria', c.preco, c.alteracoes_permitidas, 
     ...
     listarCestasAtivas(cesta) {
          return new Promise((resolve, reject) => {
            ...
            WHERE c.cesta_excluida = ? AND c.status = ? GROUP BY c.id`,
                    [cesta._cesta_excluida, cesta._status],
                    (err, results) => {
                         if (err) {
                              console.log(err.message);
                              reject(err.message);
     ...
     listarCestaEspecifica(cesta) {
          return new Promise((resolve, reject) => {
               conect.query(
                    `SELECT cesta.id, cesta.imagem, cesta.descricao, cesta.id_categoria_cesta, cesta.status, cesta.preco, cesta.alteracoes_permitidas,
              ...
              AND categoria_da_cesta.id = cesta.id_categoria_cesta`,
                    [cesta._id, cesta._cesta_excluida],
                    (err, results) => {
                         if (err) {
                              console.log(err.message);
                              reject(err.message);
     ...
     atualizarCesta(cesta) {
          return new Promise((resolve, reject) => {
               conect.query(
                    `UPDATE tb_cestas SET imagem = ?, descricao = ?, id_categoria_cesta = ?, status = ?, preco = ?,
     ...
     desabilitarCesta(cesta) {
          return new Promise((resolve, reject) => {
               conect.query(
                    `UPDATE tb_cestas SET cesta_excluida = ? WHERE id = ?`,
                    [cesta._cesta_excluida, cesta._id],
                    (err, result) => {
                         if (err) {
                              console.log(err.message);
                              reject(err.message);
