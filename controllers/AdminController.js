               ...
               for (let pedi of pedidos) {
                    ...
                    for (let i = 0; i < pedidos.length; i++) {
                         if (pedi.pedido == pedidos[i].pedido) {
                              pedi.itens.push(
                                   `0${pedidos[i].quantidade_produto} - ${pedidos[i].produto}`,
                              );
                         }
                    }
...
const relatorioEntregas = (req, res, next) => {
     ...
     async function listar() {
          try {
               let pedido = new Pedido();
               let pedidos = await pedido.listarRelatorioDeProdutosEmPedidos(
                    pedido,
               );
...
const alterarStatusPlano = (req, res, next) => {
     async function alterar() {
          try {
               ...
               let resultado = await assinatura.atualizarStatusPlanoCompra(
                    assinatura,
               );
               res.send(resultado);
          } catch (error) {
...
const salvarObservacao = (req, res, next) => {
     async function salvar() {
          try {
               ...
               res.redirect(
                    `/admin/assinatura-selecionada/${observacao.id_plano_compra}`,
               );
          } catch (error) {
               console.log(error);
               res.sendStatus(400);
